...
import { hasClient } from '../types/databaseClient.js';
...
async function resetDailyWords() {
    try {
        ...
        if (words) {
            console.log('Words with dates:', words.map((w) => ({ id: w.id, word: w.word, date: w.date })));
        }
...
resetDailyWords();
//# sourceMappingURL=reset-daily-words.js.map