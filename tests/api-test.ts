...
async function testApi(): Promise<void> {
  ...
  try {
    ...
    const dailyData = await dailyResponse.json() as DailyResponse & ErrorResponse;
    ...
    if (gameId) {
      ...
      const hintData = await hintResponse.json() as HintResponse & ErrorResponse;
    ...
    if (gameId) {
      ...
      const guessData = await guessResponse.json() as GuessResponse & ErrorResponse;
    ...
    const statsData = await statsResponse.json() as StatsResponse & ErrorResponse;
    ...
    const leaderboardData = await leaderboardResponse.json() as LeaderboardResponse & ErrorResponse;
